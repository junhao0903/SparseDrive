...
class SparsePolygonOccDecoder(object):
    def __init__(
        self,
        coords_dim: int = 2,
        score_threshold: Optional[float] = None,
    ):
        super(SparsePolygonOccDecoder, self).__init__()
    ...
    def decode(
        ...
        for i in range(bs):
            ...
            scores = cls_scores[i]
            polygons = pts_preds[i, indices[i] // num_cls]
            if self.score_threshold is not None:
                category_ids = category_ids[mask[i]]
                scores = scores[mask[i]]
                polygons = polygons[mask[i]]

            polygons_np = [poly.detach().cpu().numpy() for poly in polygons]
            output.append(
                {
